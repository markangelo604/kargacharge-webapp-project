...
// Prepare images data if uploaded
$images_value = null;
if (isset($_FILES['images']) && !empty($_FILES['images']['name'][0])) {
    ...
    if (!empty($images_data)) {
        $images_value = serialize($images_data);
    }
}

// Prepare and execute insert query - NOW INCLUDING images
$sql = "INSERT INTO charging_station (location, place_type, charge_type, rate, availability_status, details, prov_id, stat_name, images) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
$stmt->bind_param("sssdssiss", $location, $place_type, $charge_type, $rate, $availability_status, $details, $prov_id, $stat_name, $images_value);
